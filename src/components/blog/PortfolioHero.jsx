...
function PortfolioHero({ P_BtnText, H_Text, M_Text }) {
  const formattedText =
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        <motion.div
          className="w-full md:w-[60%] flex flex-col items-start gap-4 text-left"
          initial={{ x: -50, opacity: 0 }}
          whileInView={{ x: 0, opacity: 1 }}
        ...
        {/* Right Image */}
        <motion.div
          className="w-full hidden md:w-[40%] md:flex items-start justify-start"
          initial={{ x: 50, opacity: 0 }}
          whileInView={{ x: 0, opacity: 1 }}
          transition={{ duration: 1, delay: 0.5 }}
          viewport={{ once: true }}
